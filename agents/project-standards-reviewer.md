...
- **Broken cross-references** -- stale or non-existent agent names, especially old `ce-`-prefixed aliases for agents that now live under plain names such as `learnings-researcher`. Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
...
- **Protected artifact violations** -- findings, suggestions, or instructions that recommend deleting or gitignoring files in paths the standards designate as protected (e.g., `.ai/brainstorms/`, `.ai/plans/`, `.ai/solutions/`).

...
1. **Analyze First**: Search QMD for existing documentation on this topic (`mcp__qmd__query` against `docs` collection — for subagents without MCP: `qmd query "topic"` via Bash) to extend rather than duplicate. Then thoroughly examine the code, APIs, or features to understand their purpose, functionality, and usage patterns. Identify the key concepts that need explanation.

...
- **Read only the paths the orchestrator gave you.** Use your platform's file-read tool on each `path`; content-search over the supplied scratch files is fine for locating keywords. Do not read source session files directly under `~/.claude/projects/`, `~/.codex/sessions/`, or `~/.cursor/projects/` — those are MB-scale and would blow the context window. The orchestrator already extracted what's relevant.
- **Never load or invoke another skill, run extraction scripts, or use any discovery primitive.** Skill loading can deadlock in sub-agent context, and discovery/extraction is the orchestrator's job — your contract is "read the paths you were given and synthesize."

...
Adapted from pstack to discover the current host's tools and connectors without fixed model names or Cursor-only paths.
...
Before spawning investigators, inspect the current host's available tools, connectors, apps, and MCP servers. Use the host's tool catalog or discovery mechanism. Do not assume a Cursor-specific directory or tool name.
...
Use the host's available delegation feature to run investigators concurrently, within its concurrency limit. Batch the remaining categories if needed. Do not require a particular model or provider. Give each investigator access to the connector it needs while explicitly prohibiting file edits and external mutations. If the host's read-only mode removes connector access, use an agent mode that retains the connector but keep the task itself read-only.
...
Assign one investigator per available evidence category when concurrency allows. Each owns one source or connector. For a small investigation, the main agent may cover multiple categories directly when separate workers would add overhead without improving coverage.
...
Synthesize in the main thread so the evidence and user constraints stay together. A fresh-eyes synthesizer subagent is optional for broad or high-stakes investigations. Do not require a particular model or provider. Any synthesizer remains read-only but may use connectors to spot-check citations.

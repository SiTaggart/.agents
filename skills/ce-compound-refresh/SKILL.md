...
description: Refresh stale learning and pattern docs under .ai/solutions/ by reviewing them against the current codebase, then updating, consolidating, or deleting drifted ones. Use when the user asks to "refresh my learnings", "audit .ai/solutions/", "clean up stale learnings", or "consolidate overlapping docs", or when ce-compound flags an older doc as superseded. Do not trigger for general refactor, debugging, or code-review work unless the user has explicitly pointed at .ai/solutions/.
argument-hint: "[optional: scope hint — directory, filename, module, or keyword] [mode:headless] "
---

# Compound Refresh

Maintain the quality of `.ai/solutions/` over time. This workflow reviews existing learnings against the current codebase, then refreshes any derived pattern docs that depend on them.
...
10. **Delete, don't archive.** There is no `_archived/` directory. When a doc is no longer useful, delete it. Git history preserves every deleted file — that is the archive. A dedicated archive directory creates problems: archived docs accumulate, pollute search results, and nobody reads them. If someone needs a deleted doc, `git log --diff-filter=D -- .ai/solutions/` will find it.

## Scope Selection

Start by discovering learnings and pattern docs under `.ai/solutions/`.

Exclude:

- `README.md`
- `.ai/solutions/_archived/` (legacy — if this directory exists, flag it for cleanup in the report)

Find all `.md` files under `.ai/solutions/`, excluding `README.md` files and anything under `_archived/`. If an `_archived/` directory exists, note it in the report as a legacy artifact that should be cleaned up (files either restored or deleted).
...
1. **Directory match** — check if the argument matches a subdirectory name under `.ai/solutions/` (e.g., `performance-issues`, `database-issues`)
...
```text
No candidate docs found in .ai/solutions/.
...
After reviewing the underlying learning docs, investigate any relevant pattern docs under `.ai/solutions/patterns/`.
...
**Legacy cleanup** (if `.ai/solutions/_archived/` exists):
...
|---------|---------------|
| On main/master | Create a branch named for what was refreshed (e.g., `.ai/refresh-auth-and-ci-learnings`), commit, attempt to open a PR. If PR creation fails, report the branch name. |
...
1. Create a branch, commit, and open a PR (recommended) — the branch name should be specific to what was refreshed, not generic (e.g., `.ai/refresh-auth-learnings` not `.ai/compound-refresh`)
...
2. Assess whether an agent reading the instruction files would learn three things:
   ...
   This is a semantic assessment, not a string match. The information could be a line in an architecture section, a bullet in a gotchas section, spread across multiple places, or expressed without ever using the exact path `.ai/solutions/`. Use judgment — if an agent would reasonably discover and use the knowledge store after reading the file, the check passes.

...
Classify the document by reading its **content shape**, not its file path. Path is a tie-breaker hint, not the primary signal — a brainstorm-style doc placed under `.ai/plans/` should still classify as `requirements`, and a plan-shaped doc under `.ai/brainstorms/` should still classify as `plan`. The reviewers below operate differently depending on this classification, so misclassifying a plan-shaped doc as a requirements doc (or vice versa) produces noisy or under-scrutinized findings.
...
- Frontmatter fields like `type: feat|fix|refactor`, `origin: .ai/brainstorms/...`

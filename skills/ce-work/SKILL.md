...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - When auto-detecting the latest plan (blank invocation), glob `.ai/plans/*.md` AND `.ai/plans/*.html` and pick the most recent regardless of extension.
...
**Tier 2 is two steps — review, then fix.** `ce-review` is review-only. It returns findings (markdown or `mode:agent` JSON); it never edits the checkout, commits, or applies fixes.

When Tier 2 applies:

1. **Review** — Invoke the `ce-review` skill (invocation command in `references/review-findings-followup.md` § Fallback). Use `mode:agent` in orchestrated workflows; pass `plan:<path>` when you have a plan and `base:<ref>` when the merge base is already known.

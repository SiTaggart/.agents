...
3. **Code Review** (REQUIRED)
   ...
   **Tier 2 -- `ce-review` (escalation).** Invoke the `ce-review` skill with `mode:autofix`, passing `plan:<path>` when known. Then proceed to the Residual Work Gate.
...
**Tier 2 -- `ce-review` (escalation).** Invoke `ce-review mode:autofix` with `plan:<path>` when available. Safe fixes are applied automatically; residual work routes through the Residual Work Gate.

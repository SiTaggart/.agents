...
- Any durable decisions made during the dialogue can flow naturally to
  downstream artifacts (`ce-plan`, the commit message, `.ai/solutions/`)
...
`.ai/solutions/` if it's a pattern worth carrying) without a brainstorm
...
- **Success Criteria** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("document-review can act on this
...
- **`date`** — creation date in ISO 8601 (`YYYY-MM-DD`), ASCII digits only.
  Used in the filename (`.ai/brainstorms/YYYY-MM-DD-<topic>-requirements.<md|html>`).
- **`topic`** — kebab-case slug identifying the brainstorm subject (e.g.,
  ...
  Phase 0.1 scans `.ai/brainstorms/` for an existing artifact to continue.

...
|---|---|---|
| Save | `.ai/ideation/YYYY-MM-DD-<topic>-ideation.md` | Proof |
...
2. **Open and iterate in Proof** — save the ideation to Proof and enter the proof skill's HITL review loop: iterate via comments in the Proof editor; reviewed edits sync back to `.ai/ideation/` in repo mode.
...
- "Save to `.ai/ideation/` instead" (repo-mode default destination, available when CWD is inside a git repo)

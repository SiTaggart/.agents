...
4. **Save and report.** Write the deliverable to a durable, repo-tracked location — default to a sensible `.ai/` subpath (or a path the user named at the checkpoint) — and report its absolute path so the user can find it. Whether to git-commit vs. leave it written is the user's call; offer, don't force.

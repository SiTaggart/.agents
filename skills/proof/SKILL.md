...
write to a Proof doc must be attributed. Two fields carry the agent's identity:

- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:agent` — stable, lowercase-hyphenated, machine-parseable. Appears in marks, events, and the API response.
- **Display name (`name` on `POST /presence`):** `structured agent workflow` — human-readable, shown in Proof's presence chips and comment-author badges.

Set the display name once per doc session by posting to presence with the `X-Agent-Id` header; Proof binds the name to that agent ID for the session. These values are the defaults for any caller of this skill; callers running HITL review (`references/hitl-review.md`) may pass a different `identity` pair if a distinct sub-agent should own the doc. Do not use branded or ad-hoc variants — identity stays uniform unless a caller explicitly overrides it.
...
- Header: `X-Agent-Id: ai:agent` (required for presence; include on ops for consistent attribution)
...
**Comment on text:**
```json
{"type": "comment.add", "quote": "text to comment on", "by": "ai:agent", "text": "Your comment here", "baseToken": "<token>"}
```

**Reply to a comment:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:agent", "text": "Reply text", "baseToken": "<token>"}
```

**Reply and resolve in one mutation:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:agent", "text": "Fixed.", "resolve": true, "baseToken": "<token>"}
```

**Batch existing-thread comment mutations:**
```json
{"by": "ai:agent", "baseToken": "<token>", "operations": [
...
```json
{"type": "comment.resolve", "markId": "<id>", "by": "ai:agent", "baseToken": "<token>"}
{"type": "comment.unresolve", "markId": "<id>", "by": "ai:agent", "baseToken": "<token>"}
```

**Suggest a replacement (pending — user must accept/reject):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:agent", "content": "replacement text", "baseToken": "<token>"}
```

**Suggest and immediately apply (tracked but committed — user can reject to revert):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:agent", "content": "replacement text", "status": "accepted", "baseToken": "<token>"}
```
...
```json
{"type": "suggestion.accept", "markId": "<id>", "by": "ai:agent", "baseToken": "<token>"}
{"type": "suggestion.reject", "markId": "<id>", "by": "ai:agent", "baseToken": "<token>"}
```
...
```json
{"type": "rewrite.apply", "content": "full new markdown", "by": "ai:agent", "baseToken": "<token>"}
```
...
**Required headers:**
- `X-Agent-Id: ai:agent` (identity for presence; keep aligned with `by`)
...
| POST | `/marks/suggest-replace` | `{"quote":"old","by":"ai:agent","content":"new"}` |
| POST | `/marks/suggest-insert` | `{"quote":"after this","by":"ai:agent","content":"insert"}` |
| POST | `/marks/suggest-delete` | `{"quote":"delete this","by":"ai:agent"}` |
| POST | `/marks/comment` | `{"quote":"text","by":"ai:agent","text":"comment"}` |
| POST | `/marks/reply` | `{"markId":"<id>","by":"ai:agent","text":"reply"}` |
| POST | `/marks/resolve` | `{"markId":"<id>","by":"ai:agent"}` |
| POST | `/marks/accept` | `{"markId":"<id>"}` |
| POST | `/marks/reject` | `{"markId":"<id>"}` |
| POST | `/rewrite` | Last-resort whole-doc replacement: `{"content":"full markdown","by":"ai:agent"}` |
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"text",by:"ai:agent",text:"comment",baseToken:$base}')")
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "X-Agent-Id: ai:agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:agent",content:"new",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "X-Agent-Id: ai:agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:agent",content:"new",status:"accepted",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
curl -X POST "https://www.proofeditor.ai/api/agent/abc123/edit/v2?return=minimal" \
  ...
  -H "X-Agent-Id: ai:agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$EDIT_BASE" '{by:"ai:agent",baseToken:$base,operations:[{op:"find_replace_in_doc",find:"old",replace:"new",occurrence:"all"}]}')"
```
...
curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/presence" \
  ...
  -H "x-share-token: $TOKEN" \
  -H "X-Agent-Id: ai:agent" \
  -d '{"name":"structured agent workflow","status":"reading","summary":"Uploaded doc"}'
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/ops" \
  ...
  -H "X-Agent-Id: ai:agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"Content here",by:"ai:agent",text:"Added a note",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
curl -X POST "https://www.proofeditor.ai/api/agent/$SLUG/edit/v2?return=minimal" \
  ...
  -H "X-Agent-Id: ai:agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$EDIT_BASE" '{by:"ai:agent",baseToken:$base,operations:[{op:"find_replace_in_doc",find:"Content",replace:"Updated content",occurrence:"all"}]}')"
```
...
- Always include `by: "ai:agent"` on every op and `X-Agent-Id: ai:agent` in headers for consistent attribution
